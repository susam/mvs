...
The program [mvs.py][] available in this project consumes text via
...
by consuming the text in the file [book.txt](book.txt).
...
Here are some usage examples of these command line arguments:
  ...
  1. Generate gibberish using a trigram model:
     ...
     ```sh
     python3 mvs.py 3 < book.txt
     ```

  2. Generate gibberish up to 250 words long:

     ```sh
     python3 mvs.py 2 250 < book.txt
     ```
...
```
$ python3 mvs.py 2 100 'At last' < book.txt
...
```
$ make filter-website && python3 mvs.py < susam.txt
